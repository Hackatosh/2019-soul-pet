        ...
        return (
            <Modal onHide={this.props.onHide} show={this.props.show} aria-labelledby="contained-modal-title-vcenter" centered>
                <Modal.Header closeButton>
                    <Modal.Title id="contained-modal-title-vcenter">{this.props.event === undefined ? "Ajouter un évènement" : "Modifier un évènement" }</Modal.Title>
                </Modal.Header>
                <Formik onSubmit={values => {
                    ...
                    // Else we are editing an existing event
                        ...
                        initialValues={this.props.event === undefined ?
                            { name: '', beginDate: '', endDate: '', description:'', userId:AuthenticationService.User.id } :
                            { name: this.props.event.name, beginDate: this.props.event.beginDate.toISOString().substr(0, 10), endDate: this.props.event.endDate.toISOString().substr(0, 10), description: this.props.event.description }}>
                    {props => (
                        <Form onSubmit={props.handleSubmit}>
                            <Modal.Body>
                                ...
                                <Form.Group controlId="eventName">
                                    ...
                                    <Form.Control name="name" type="text" placeholder="Entrez le nom de l'évènement" onChange={props.handleChange} value={props.values.name} required />
                                </Form.Group>
                                <Form.Group controlId="eventBeginDate">
                                    <Form.Label>Quand commence-t-il ?&nbsp;?</Form.Label>
                                    <Form.Control type="date" name="beginDate" onChange={props.handleChange} value={props.values.beginDate} required />
                                </Form.Group>
                                <Form.Group controlId="eventEndDate">
                                    <Form.Label>Quand se termine-t-il&nbsp;?</Form.Label>
                                    <Form.Control type="date" name="endDate" onChange={props.handleChange} value={props.values.endDate} required />
                                </Form.Group>
                            ...
                            <Modal.Footer>
                                <Button variant="primary" type="submit">{this.props.event === undefined ? 'Ajouter' : 'Éditer'}</Button>
